        ...
        <td>
            <div class="btn-group" role="group">
                ...
                @if(!Auth::user()->isMember())
                    ...
                    @if($plan->is_default)
                        <a href="{{ route('workout-plans.assign.form', $plan->id) }}" class="btn btn-sm btn-success" title="Assign">
                            <i class="fas fa-user-plus"></i> <span class="d-none d-md-inline">Assign</span>
                        </a>
                    @endif
                    <button type="button" class="btn btn-sm btn-danger" title="Delete" 
                        data-delete-url="{{ route('workout-plans.destroy', $plan->id) }}"
                        data-delete-name="{{ $plan->name }}"
                        data-delete-type="Workout Plan"
