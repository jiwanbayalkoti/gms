...
class DashboardController extends Controller
{
    ...
    private function apiAdminDashboard($data)
    {
        ...
        $data['recentPayments'] = $recentPaymentsQuery->get()->map(function($payment) {
            return [
                'id' => $payment->id,
                'member_name' => $payment->member ? $payment->member->name : 'N/A',
                'amount' => $payment->amount,
                'date' => $payment->payment_date ? Carbon::parse($payment->payment_date)->format('M d, Y') : null,
                'payment_date' => $payment->payment_date,
            ];
        });
        
        // Recent salary payments (Expenses)
        $recentSalaryPaymentsQuery = SalaryPayment::with('employee')
        ...
        $data['recentSalaryPayments'] = $recentSalaryPaymentsQuery->get()->map(function($salaryPayment) {
            return [
                'id' => $salaryPayment->id,
                'employee_name' => $salaryPayment->employee ? $salaryPayment->employee->name : 'N/A',
                'amount' => $salaryPayment->net_amount,
                'date' => $salaryPayment->payment_date ? Carbon::parse($salaryPayment->payment_date)->format('M d, Y') : null,
        ...
        for ($i = 6; $i >= 0; $i--) {
            $date = Carbon::now()->subDays($i);
            $attendanceLast7Days[] = $date->format('D');
            
            $attendanceData[] = Attendance::whereDate('check_in_time', $date->toDateString())->count();
        }
        
        $data['attendanceChartLabels'] = $attendanceLast7Days;
        $data['attendanceChartData'] = $attendanceData;
        
        // Format upcoming classes for API
        $data['upcomingClasses'] = $data['upcomingClasses']->map(function($class) {
            return [
                'id' => $class->id,
                'name' => $class->name,
                'trainer_name' => $class->trainer ? $class->trainer->name : 'N/A',
                'date_time' => $class->class_date && $class->class_time 
                    ? Carbon::parse($class->class_date . ' ' . $class->class_time)->format('M d, Y h:i A')
                    : null,
